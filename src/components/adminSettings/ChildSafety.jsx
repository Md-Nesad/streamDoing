...
import UpdateChildSafety from "./UpdateChildSafety";
...
export default function ChildSafety() {
  ...
  const { data, loading, error } = useFetch(
    `${BASE_URL}/admin/settings/child-safety`,
    refresh,
  );
  ...
  return (
    <div className="py-4 bg-[#FFFFFF] rounded-md shadow-[0_2px_10px_rgba(0,0,0,0.06)] w-full overflow-x-auto mt-7 mb-10">
      <div className="flex items-center justify-between mb-4 px-3 sm:px-5">
        <h2 className="text-xl font-semibold">Child Safety</h2>
      ...
      <table className="w-full text-left border-collapse text-nowrap">
        <thead>
          <tr className="text-[#535353] text-md font-medium">
            <th className="p-3 pl-10">Child Safety Policy</th>
        ...
        <tbody>
          <tr className="border-t border-[#DFDFDF] hover:bg-gray-50 text-md">
            <td className="p-3 pl-10">{data?.childSafty?.content}</td>
