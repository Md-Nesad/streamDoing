...
const UpdateChildSafety = ({ onClose, onSuccess }) => {
  const [loading, setLoading] = useState(false);
  const [content, setContent] = useState("");
  const handleSubmit = useJsonPost(`${BASE_URL}/admin/settings/child-safety`);
  ...
  const handleCreate = async () => {
    try {
      ...
      console.log(result);
      toast.success("Child safety updated.");
      onClose();
      onSuccess();
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 max-sm:px-3 ">
      <div className="bg-white rounded-xl p-4 sm:p-6 w-[600px] shadow-lg animate animatefadeIn">
        <h2 className="text-lg font-semibold mb-6">Update Child Safety</h2>
        ...
        <div className="w-full">
          ...
          <textarea
            ...
            placeholder="Enter child safety policy content..."
            rows={4}
...
export default UpdateChildSafety;
